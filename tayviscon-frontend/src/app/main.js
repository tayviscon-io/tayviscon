...
function removeNavClasses() {
    $('#scope').removeClass('why-scope edu-scope projects-scope community-scope');
...
$('#mobile-edu').click(function () {
    if ($('#mobile-edu-items').is(':hidden')) {
        resetSlide();
    }
    $('> .mobile-nav-arrow', this).toggleClass('flipped');
    $('#mobile-edu-items').slideToggle();
});
